...
void Petri_Controller_2::execute(void* arg) {
	...
	while (!isStopped()) {
		...
		if (pulse.code == PULSE_PUK_INFORMATION) {
			...
			id = puk_c2.get_id();
			typ = puk_c2.get_typ();
			hoehenmessung1 = puk_c2.get_hoehenmessung1();
		}
...
void Petri_Controller_2::process_transitions() {
	...
	if (places[1] && !places[2] && (petri_controller_2_inputs[WERKSTUECK_IN_HOEHENMESSUNG] == true) && (Puk_added_LSH_c2_timeout == true) && (notaus
			== false)) {
		...
		puk_tmp_hoehe = petri_controller_2_sen->getHeight();

		if ((puk_c2.get_typ() == PUK_GROSS) && (puk_tmp_type == PUK_GROSS)) {
			puk_c2.set_hoehenmessung2(petri_controller_2_sen->getHeight());
			hoehenmessung2 = puk_c2.get_hoehenmessung2();
			aussortieren = true;
		} else {
			if(puk_c2.get_typ()==PUK_METALL){
				hoehenmessung2 = puk_tmp_hoehe;
			}else{
				puk_c2.set_hoehenmessung2(puk_tmp_hoehe);
				puk_c2.set_typ(puk_tmp_type);
				typ = puk_tmp_type;
				hoehenmessung2 = puk_tmp_hoehe;
			}
	...
	if (places[3] && !places[4] && (petri_controller_2_inputs[WERKSTUECK_IN_WEICHE] == true) && (Puk_added_LSW_c2_timeout == true) && (notaus
			== false)) {
	...
	if (places[6] && !places[7] && (petri_controller_2_inputs[AUSLAUF_WERKSTUECK] == true) && (Puk_added_LSEND_c2_timeout == true) && (notaus
			== false)) {
	...
	if (places[7] && !places[0] && (petri_controller_2_inputs[AUSLAUF_WERKSTUECK] == false) && (notaus == false)) {
		...
		if(typ == 1){
			printf("PUK TYP: PUK LOCH \n");fflush(stdout);
		}

		if(typ == 2){
			printf("PUK TYP: PUK Metall \n");fflush(stdout);
		}

		printf("PUK HOEHEN-MESSWERT BAND 1: %d \n",hoehenmessung1);fflush(stdout);
		printf("PUK HOEHEN-MESSWERT BAND 2: %d \n",hoehenmessung2);fflush(stdout);
	}
	...
	if (places[1] && !places[18] && (petri_controller_2_inputs[WERKSTUECK_IN_HOEHENMESSUNG] == true) && (Puk_added_LSH_c2_timeout == false)
			&& (first_chance == true) && (notaus == false)) {
	...
	if (places[3] && !places[18] && (petri_controller_2_inputs[WERKSTUECK_IN_WEICHE] == true) && (Puk_added_LSW_c2_timeout == false) && (notaus
			== false)) {
	...
	if (places[6] && !places[18] && (petri_controller_2_inputs[AUSLAUF_WERKSTUECK] == true) && (Puk_added_LSEND_c2_timeout == false) && (notaus
			== false)) {
